...
const _doi_on_page = Object.create(null);
...
function append_i3t (pmid, S, sec) {
  ...
  setTimeout(function () {
    ...
    if (impact3 !== null && impact3.textContent.indexOf(' [Preprint]') > 0) {
      impact3.textContent = impact3.textContent.split(' [Preprint]')[0];
...
function process_googlescholar () { // 2018 Oct
  ...
  let i; let ilen; let tmp; const nodes = byID('gs_res_ccl_mid').childNodes; let a; let b; let c; const d = [];
  for (i = 0, ilen = nodes.length; i < ilen; i += 1) {
    if (!nodes[i]) { continue; }
    a = nodes[i].textContent;
    if (!nodes[i].lastChild || !a) { continue; }
    if (a.indexOf(' Cited by ') === -1) {
      c = 0;
    } else {
      b = a.split(' Cited by ')[1];
      if (b.indexOf('Related article') === -1) {
...
async function copyToTheClipboard (textToCopy) {
  const el = page_d.createElement('textarea');
  el.value = textToCopy;
  el.setAttribute('readonly', '');
  el.style.position = 'absolute';
  el.style.left = '-9999px';
  page_d.body.appendChild(el);
  el.select();
  page_d.execCommand('copy');
  page_d.body.removeChild(el);
}
...
function insert_clippy (ID, t_cont, _obj, multi_left = false) {
  ...
  if (t_cont.indexOf('Among authors: ') > 0) {
    const _tt = t_cont.split('Among authors: ');
    t_cont = _tt[0] + _tt[1].substr(_tt[1].indexOf('.') + 2);
  }
  if (!_obj) {
    copyToTheClipboard(t_cont);
    return;
  }
  ...
  b.id = 'clippy' + ID;
  b.onclick = function () { copyToTheClipboard(t_cont); };
  _obj.appendChild(b);
}
...
function new_pubmed_single1 (not_first) {
  ...
  y.onclick = function () { copyToTheClipboard(t_cont); };
...
function new_pubmed_single () {
  ...
  yy.onclick = function () { copyToTheClipboard(t_cont); };
...
function new_pubmed_multi1 (zone, num, ajax = false) {
  ...
  if (regdoi.test(t_cont) || doipattern.test(t_cont)) {
    _doi_on_page[ID] = doipattern.exec(t_cont)[1];
  }
...
function prep_call (pmids) {
  ...
  if (need_insert && !byID('pl4_title')) {
    ...
    if (byID('full-view-heading') !== null) { // new single
...
function parse_page_div (ajax = true) {
  if (ajax) {
    ...
    if (byID('thepaperlink_pubmederAll') !== null &&
        byID('thepaperlink_pubmederAll').textContent === f4a) {
      return;
    }
  }
  f4 && clearInterval(f4);
  let i; let len;
  let legacy_May2020 = false; // https://www.nlm.nih.gov/pubs/techbull/so20/so20_pubmed_update.html
  ...
  for (i = 0, len = byTag('div').length; i < len; i += 1) {
    ...
    //  legacy_pubmed('div', i);
    //  legacy_May2020 = true;
    if (byTag('div')[i].className === 'article-source' && byID('full-view-journal-trigger')) {
...
function get_request (msg) {
  ...
  if (msg.local_mirror && msg.local_mirror !== '127.0.0.1') {
    ...
    byID('pl4_title').innerHTML = old_title +
        ...
        '" target="_blank">[?]</a>';
    if (msg.except.indexOf('Guest usage limited') === -1) {
      byID('thepaperlink_alert').onclick = function () {
        const answer = confirm('\n do you want to alert this issue to the developer?\n');
        if (answer) {
          alert_dev(msg.tpl);
        }
      };
    ...
    try {
      if (msg.g_num === 1 && msg.g_link === 1) {
        ...
        byID('citedBy' + msg.pmid).textContent = ''; // No citation
  ...
  for (i = 0; i < r.count; i += 1) {
    ...
    if (!msg.except) {
      ...
      if (impact3 !== null) {
        ...
        if (i3t.indexOf(' Actions') > 0) { // new abstract page, 2020-2-23
          ...
          impact3.innerHTML = i3t + '<span><span style="background:#e0ecf1;padding:0 1px 0 1px"></span></span>';
    ...
    if (!msg.except && r.item[i].doi) {
      ...
      if (_doi_on_page[pmid]) { // 2022-3-7 2022-5-7 2022-5-18
        ...
        if (!msg.except) {
          a_proxy({ money_emailIt: pmid, doi: _doi_on_page[pmid], new_doi: 1 });
        }
      }
      div_html += tmp;
    }
    if (!msg.except && r.item[i].pii && byID('citedBy' + pmid)) { // @@@@
