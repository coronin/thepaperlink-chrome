...
function run() {
  var i, len, z;
  try {
    search_term = byID('term').value; // 2013-3-26
  } catch (err) {
  ...
  for (i = 0, len = byTag('div').length; i < len; i += 1) {
    if (byTag('div')[i].className === 'rprt' || byTag('div')[i].className === 'rprt abstract') {
      getPmid('div', i);
    } else if (!search_term && byTag('div')[i].className === 'print_term') {
      z = byTag('div')[i].textContent;
      if (z) {
  ...
  for (i = 0; i < r.count; i += 1) {
    ...
    if (r.item[i].doi) {
      ...
      tmp = '<a id="thepaperlink_doi' + pmid +
          ...
          ) + '" target="_blank">publisher</a><a id="thepaperlink_shark' + pmid +
          '" href="https://shark-bite.io/' + uneval_trim(r.item[i].doi) +
          '#" target="_blank">&#x219d;</a>';
      div_html += tmp;
    } else if (r.item[i].pii) {
      tmp = '<a id="thepaperlink_pii' + pmid +
          ...
          '" href="https://shark-bite.io/retrieve/pii/' + uneval_trim(r.item[i].pii) +
          '" target="_blank">&#x219d;</a>';
      div_html += tmp;
    }
