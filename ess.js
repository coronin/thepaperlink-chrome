...
function peaks (name) {
  ...
  var tpl = localStorage.getItem('thepaperlink_apikey') || '';
...
function eFetch (pmid) {
  ...
  $.getJSON(url, args, function (d) {
    ...
    var l = d.result.PubmedArticle[0]; var tmp; var j; var len;
...
function eSS (search_term, tabId) {
  ...
  $.get(url,
    function (xml) {
      ...
      var a = $(xml).find('IdList'); // cannot use Count
      var b = 0;
      a.each(function () {
        b += 1;
      });
